...
def main():
    ...
    if previous.get('source',{}).get('url') != 'https://data.bodik.jp/ja/dataset/270008_event' or any(e.get('properties',{}).get('fictional') for e in previous.get('features',[])):
        raise ValueError('公開用のBODIKイベントデータが必要です。npm run update:events を実行してください。')
